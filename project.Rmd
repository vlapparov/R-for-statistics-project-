...
As we can see, there is no fort correlation between alcohol consumption and the academic results. 


#Towards a real mathematical model

First, let's convert our data to something, that we can use for model constructing.
...
So we will choose only variables with the level of correlation more than 0.15 for our purposes.
...
We see that R-square is not very high (0.13), but this is not as bad as estimated: we build a model, based on the data, where we manually changed data.
...
Take a look at the following biplot of PC1 and PC2 (explaining 50% of the data):
